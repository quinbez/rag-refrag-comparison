...
def run_benchmark(validation_set, retriever, refrag_retriever, llm, max_queries=None):
    ...
    print(f"Total queries run: {n}")
    print(f"Average RAG time per query: {avg_rag_time:.2f}s, EM: {total_rag_em/n:.2%}, F1: {total_rag_f1/n:.2%}")
    print(f"Average REFRAG time per query: {avg_refrag_time:.2f}s, EM: {total_refrag_em/n:.2%}, F1: {total_refrag_f1/n:.2%}")
